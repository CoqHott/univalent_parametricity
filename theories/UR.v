...
Arguments pr _ {_ _ _} _ _.

Notation "x ≈p y" := (pr plain x y) (at level 20).

Notation "x ≈u y" := (pr univalent x y) (at level 20).

Notation "x ≈ y" := (pr _ x y) (at level 20).
...
#[universes(collapse_sort_variables=no)]
Instance PR_Type_plain_univ (A B : Type) (H: A ≈u B) : PR plain A B := Ur H. 

#[universes(collapse_sort_variables=no)]
Instance PR_Type_univ_univ (A B : Type) (H: A ≈u B) : PR univalent A B :=
...
Definition PR_Type_gen k (A B:Type) (H:@pr _ _ _ (PR_Type k) A B) : PR k A B :=
  ...
  | plain => fun H => H
  | univalent => fun H => PR_Type_univ_univ _ _ H
  end H.
...
#[universes(collapse_sort_variables=no)]
Instance PR_Prop_plain_univ (A : Prop) (B : SProp) (H: A ≈u B) : PR plain A B := Ur_P H. 

#[universes(collapse_sort_variables=no)]
Instance PR_Prop_univ_univ (A : Prop) (B : SProp) (H: A ≈u B) : PR univalent A B :=
...
Definition ur_refl {A B: Type} (e : A ≈u B) :forall a : A, a ≈ ↑ a.
Proof.
...
Proof.
  intros a b. cbn. 
  refine (transport_eq_gen (fun X => (a = univalent_transport b) ↔ (a ≈ X))
                       (e_sect _ b) _). 
...
Proof.
  ...
  refine (transport_eq_gen (fun X => (a = X) ↔ (a ≈ univalent_transport a'))
                       (e_sect _ a') _). 
...
Definition URForall k A A' (B : A -> Type) (B' : A' -> Type) {HA : PR k A A'} 
           {HB: forall x y (H: x ≈ y), PR k (B x) (B' y)} : PR k (forall x, B x) (forall y, B' y)
  :=
  {| pr := fun f g => forall x y (H:x ≈ y), f x ≈ g y |}.

#[export] Hint Extern 0 (PR ?k (forall x:?A, _) (forall x:?A', _)) =>
  erefine (@URForall_Type k A A' _); cbn in *; intros : typeclass_instances.

#[export] Hint Extern 1 (PR ?k (forall x:?A, _) (forall x:?A', _)) =>
  erefine (@URForall k A A' _ _ _ _); cbn in *; intros : typeclass_instances.
...
Proof.
  ...
  - econstructor.
    intros a a'. cbn. unfold univalent_transport. 
    refine (transport_eq_gen (fun X => _ ↔ (a ≈ X)) (e_retr' H _)^ _). apply ur_coh.
Defined.     
...
Definition UR_Equiv_gen (X:Type) (eX : X ≈ X) (A B: X -> Type) (HAB: forall x, B x ≃ A x) (x y:X) (e : x ≈ y) (H:A x ≈p A y)
  : B x ≈p B y.
Proof.
...
Definition UR_Type_Equiv_gen (X:Type) (eX : X ≈ X) (A B: X -> Type) (HAB: forall x, B x ≃ A x) (x y:X) (e : x ≈ y) (H:A x ≈u A y)
  (H':A x ≈u A y)
  : B x ≈u B y.
Proof.
